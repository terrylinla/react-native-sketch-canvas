...
class SketchCanvas extends React.Component {
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        this.setState({
            text: this._processText(
                nextProps.text
                    ? nextProps.text.map((t) => Object.assign({}, t))
                    : null
            ),
        });
    }
    ...
    addPath(data) {
        if (this._initialized) {
            if (
                this._paths.filter((p) => p.path.id === data.path.id).length ===
                0
            )
            ...
            const pathData = data.path.data.map((p) => {
                const coor = p
                    .split(",")
                    .map((pp) => parseFloat(pp).toFixed(2));
                return `${
                    (coor[0] * this._screenScale * this._size.width) /
                    data.size.width
                },${
                    (coor[1] * this._screenScale * this._size.height) /
                    data.size.height
                }`;
            });
            UIManager.dispatchViewManagerCommand(
            ...
            this._pathsToProcess.filter((p) => p.path.id === data.path.id)
            ...
            onPanResponderMove: (evt, gestureState) => {
                ...
                if (this._path) {
                    UIManager.dispatchViewManagerCommand(
                        ...
                        [
                            parseFloat(
                                (gestureState.moveX - this._offset.x).toFixed(
                                    2
                                ) * this._screenScale
                            ),
                            parseFloat(
                                (gestureState.moveY - this._offset.y).toFixed(
                                    2
                                ) * this._screenScale
                            ),
                        ]
    ...
    fill(startX: number, startY: number, width: number, height: number) {
        this._path = {
            id: parseInt(Math.random() * 100000000),
            color: this.props.strokeColor,
            width: 1,
